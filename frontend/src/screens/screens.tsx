...
export function OverviewScreen({ data, actions }: { data: FarmData; actions: ScreenActions }) {
  ...
  return <>
    <PageHeader eyebrow="Live farm read model" title="Farm overview" description="The active grow, current conditions, exceptions, and equipment state in one operational view." actions={<button className="gn-button" onClick={actions.refreshSimulator}><RefreshCw size={16} /> Refresh simulator</button>} />
    <div className="gn-callout"><div><ShieldCheck /><span><strong>Browser simulator is active.</strong> Commands and acknowledgements on this screen are local simulations, never claims about physical equipment.</span></div><Status tone="simulated">Simulated control</Status></div>
    ...
    <div className="gn-dashboard-grid">
      ...
      <Card title="Equipment" subtitle="Acknowledged simulator state only">
...
export function GrowCyclesScreen({ data, actions }: { data: FarmData; actions: ScreenActions }) {
  ...
  return <><PageHeader eyebrow="Crop execution" title="Grow cycles" description="Stage-aware recipe targets, positions, observations, inputs, alerts, and harvest history remain attached to one production record." actions={<button className="gn-button primary"><Plus size={16} /> Plan grow</button>} />
...
export function TwinScreen({ data, selection, actions }: { data: FarmData; selection?: Selection; actions: ScreenActions }) {
...
export function AutomationScreen({ data, actions }: { data: FarmData; actions: ScreenActions }) {
  return <><PageHeader eyebrow="Bounded and auditable rules" title="Automation" description="Rules propose actions through the same validation path as manual commands. Browser runtime supports observe and simulate modes only." actions={<button className="gn-button primary"><Plus size={16} /> Create rule</button>} />
    <div className="gn-callout warning"><div><CloudOff /><span><strong>Unattended hardware execution is unavailable.</strong> Browsers may suspend timers; persistent physical schedules belong on the Go server and ESP32 edge.</span></div></div>
...
function DeviceCard({ device, data, actions }: { device: Device; data: FarmData; actions: ScreenActions }) {
  ...
  return <Card title={device.name} subtitle={`${device.type.replaceAll("_", " ")} · ${device.firmware_version}`} action={<Status tone={device.online ? device.simulated ? "simulated" : "ok" : "critical"}>{device.online ? device.simulated ? "Simulated" : "Online" : "Offline"}</Status>}>
    ...
    {channel && <div className="gn-device-control">{channel.value_type === "boolean" ? <><button className="gn-button" onClick={() => actions.command(channel.id, !device.state, "manual browser control")} disabled={!device.online}>{device.state ? <Pause size={15} /> : <Play size={15} />}{device.state ? "Turn off" : "Turn on"}</button></> : <><input aria-label={`${device.name} output`} type="range" min={channel.safe_minimum} max={channel.safe_maximum} value={output} onChange={(event) => setOutput(Number(event.target.value))} /><strong>{output}%</strong><button className="gn-button primary" onClick={() => actions.command(channel.id, output, "manual browser control")} disabled={!device.online}>Apply</button></>}</div>}
    <button className="gn-text-button" onClick={() => actions.setDeviceOnline(device.id, !device.online)}>{device.online ? "Simulate offline fault" : "Reconnect simulator"}</button>
  </Card>;
}

export function DevicesScreen({ data, actions }: { data: FarmData; actions: ScreenActions }) {
  return <><PageHeader eyebrow="Device and logical channel registry" title="Devices" description="Stable application channels survive physical replacement. Command success appears only after the simulator acknowledgement path completes." actions={<button className="gn-button" onClick={actions.refreshSimulator}><RefreshCw size={16} /> Generate telemetry</button>} />
    <div className="gn-device-grid">{data.devices.map((device) => <DeviceCard key={device.id} device={device} data={data} actions={actions} />)}</div>
...
export function SettingsScreen({ data, runtimeMode, actions }: { data: FarmData; runtimeMode: RuntimeMode; actions: ScreenActions }) {
  ...
  return <><PageHeader eyebrow="Runtime and data portability" title="Settings" description="Browser data stays in IndexedDB and can be moved or restored through the complete versioned GrowNerve archive." />
    <div className="gn-settings-grid"><Card title="Runtime" subtitle="Current application adapter"><dl className="gn-settings-list"><div><dt>Mode</dt><dd><Status tone={runtimeMode === "browser" ? "simulated" : "ok"}>{runtimeMode === "browser" ? "Browser only" : "Server"}</Status></dd></div><div><dt>Storage</dt><dd>IndexedDB</dd></div><div><dt>Application</dt><dd>0.1.0</dd></div><div><dt>Archive schema</dt><dd>Version 1</dd></div><div><dt>Records</dt><dd>{Object.values(data).reduce((sum, records) => sum + records.length, 0).toLocaleString()}</dd></div></dl></Card>
      ...
      <Card title="Safety boundary" subtitle="What browser-only operation means"><div className="gn-safety-copy"><ShieldCheck /><div><strong>Designed for planning, record keeping, demonstrations, and simulation.</strong><p>It does not provide unattended real-equipment guarantees. Use the full Go/PostgreSQL/MQTT/ESP32 deployment for persistent physical control.</p></div></div></Card></div>
  </>;
}
