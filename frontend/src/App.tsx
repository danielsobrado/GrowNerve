...
export function App({ repository, runtimeMode = "browser" }: { repository: ApplicationRepository; runtimeMode?: RuntimeMode }) {
  ...
  useEffect(() => { let active = true; void repository.load().then((value) => { if (active) setData(value); }).finally(() => { if (active) setLoading(false); }); const unsubscribe = repository.subscribe(() => { void repository.load().then((value) => { if (active) setData(value); }).catch(() => undefined); }); return () => { active = false; unsubscribe(); }; }, [repository]);
  ...
  const loadPilot = useCallback(() => { void persist(tickSimulator(pilotData(), new Date().toISOString(), 1)); setRoute("overview"); }, [persist, setRoute]);
  ...
  const actions = useMemo<ScreenActions>(() => ({
    ...
    setDeviceOnline: (deviceId, online) => { if (data) void persist(setDeviceOnline(data, deviceId, online)); },
  ...
  if (loading) return <div className="gn-loading"><div className="gn-loading-mark"><Leaf /></div><p>Opening local farm…</p></div>;
  if (!data) return <FirstRun repository={repository} onPilot={loadPilot} onCreated={setData} />;
...
function FirstRun({ repository, onPilot, onCreated }: { repository: ApplicationRepository; onPilot: () => void; onCreated: (data: FarmData) => void }) {
  ...
  return <main className="gn-welcome"><div className="gn-welcome-brand"><div><Leaf /></div><span>GrowNerve</span></div><section><p className="gn-eyebrow">Local-first farm intelligence</p><h1>Welcome to GrowNerve</h1><p>Start a private farm in this browser, explore the deterministic pilot tent, or restore a complete portable archive. Nothing is silently seeded.</p><div className="gn-welcome-actions"><button aria-label="Create local farm" className="gn-welcome-card" onClick={createLocal}><span><Plus /></span><div><strong>Create local farm</strong><p>Start empty with IndexedDB persistence.</p></div></button><button aria-label="Load pilot example" className="gn-welcome-card featured" onClick={onPilot}><span><Leaf /></span><div><strong>Load pilot example</strong><p>Explore the 3 × 3 ft DWC reference grow.</p></div></button><label className="gn-welcome-card"><span><FileUp /></span><div><strong>Import .grownerve.json</strong><p>Validate and restore a portable backup.</p></div><input type="file" accept=".json,.grownerve.json" onChange={(event) => void importArchive(event.target.files?.[0])} /></label></div>{error && <p className="gn-error">{error}</p>}<p className="gn-welcome-note"><Download size={14} /> Your data stays local and remains exportable.</p></section></main>;
}
