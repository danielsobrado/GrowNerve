...
func TestParseTelemetry(t *testing.T) {
	...
	if err != nil { t.Fatalf("ParseTelemetry(): %v", err) }
	if envelope.DeviceID != deviceID || len(envelope.Samples) != 1 { t.Fatalf("envelope = %#v", envelope) }
}
...
func TestParseTelemetryRejectsInvalidNetworkInput(t *testing.T) {
	...
	for _, payload := range tests { if _, err := ParseTelemetry(payload); err == nil { t.Fatalf("ParseTelemetry(%s) succeeded", payload) } }
}
...
func TestCommandAndAcknowledgementValidation(t *testing.T) {
	...
	if err := command.Validate(now); err != nil { t.Fatalf("valid command: %v", err) }
	command.ExpiresAt = now.Add(-time.Second)
	if err := command.Validate(now); err == nil { t.Fatal("expired command accepted") }
	ack := Acknowledgement{ProtocolVersion: 1, CommandID: deviceID, DeviceID: channelID, Result: "applied", AcknowledgedAt: now}
	if err := ack.Validate(); err != nil { t.Fatalf("valid ack: %v", err) }
	ack.Result = "unknown"
	if err := ack.Validate(); err == nil { t.Fatal("unknown ack result accepted") }
}
